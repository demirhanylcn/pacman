    ...
    public void showGame(){
        ...
        Thread th = new Thread(() -> {
            while (model.isActive()) {
                try {
                    ...
                    if(model.getPlayerAroundPacman() && model.getUpgrades().stream().noneMatch(x-> x instanceof UpgradeImmortality)){
                        model.setHealth(model.getHealth()-1);
                        model.resetPlayer();
                        view.updateBottomPanel();
                    }
                    if (model.getPlayerAroundPacman() && model.getUpgrades().stream().noneMatch(x-> x instanceof UpgradeDemon)){
                        model.setHealth(model.getHealth()-1);
                        model.setScore(model.getScore()+model.getScore()*3);
                        model.resetPlayer();
                        view.updateBottomPanel();
                    }
        ...
        Thread calculatorUpgrades = new Thread(()->{
            while (model.isActive()) {
                ...
                boolean updateBottomPanel = toRemove.size() > 0;
